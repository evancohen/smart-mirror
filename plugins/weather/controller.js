	...
	weather.weeklyForecast = function () {
		...
		case 'Climacell':
			...
			for (i=0; i<datalength; i++) {
				...
				weather.forecast.data.daily.data[i].day = i>0?moment.utc(weather.forecast.timelines[0].intervals[i].startTime, 'YYYY-MM-DD').format('ddd') : $translate.instant('forecast.today');
				weather.forecast.data.daily.data[i].temperatureMin = parseFloat(cvttof(weather.forecast.timelines[0].intervals[i].values.temperatureMin,config.forecast.units)).toFixed(0);
				weather.forecast.data.daily.data[i].temperatureMax = parseFloat(cvttof(weather.forecast.timelines[0].intervals[i].values.temperatureMax,config.forecast.units)).toFixed(0);
				weather.forecast.data.daily.data[i].wi = "wi-forecast-io-" + convert_conditions_to_icon( weather.forecast.timelines[0].intervals[i].values.weatherCode, null, null, 'utc') ;
				weather.forecast.data.daily.data[i].counter = String.fromCharCode(97 + i);
				//weather.forecast.data.daily.data[i].iconAnimation = weather.forecast.data.daily.data[i].icon;
			}
			break;
