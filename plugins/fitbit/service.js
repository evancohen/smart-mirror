(function () {
	...
	function FitbitService() {
		...
		if (typeof config.fitbit != "undefined") {
			...
			fitbit = new Fitbit(config.fitbit, function (token, callback) {
				if (token && "access_token" in token) {
					// persist the token
					persist.write(tokenFile, token, function (err) {
						if (err) {
							console.log("firbit, error persisting token");
							callback(err);
						} else callback(null);
					});
				} else callback("invalid token");
			});
		...
		service.init = function (cb) {
			...
			fs.stat(tokenFile, function (err) {
				if (err == null) {
					...
					console.error(
						"Fitbit authentication required, please visit the following link: http://localhost:4000/fitbit to authenticate your credentials.",
						err
					);
				} else {
					console.error(err);
