function Todoist($scope, TodoistService, SpeechService, Focus) {
	...
	SpeechService.addCommand('todoist_add', function (element, project) {
		...
		addTodoist(element, project);
		Focus.change("todoist");
	});

	var getTodoist = function(project){
		TodoistService.renderTasks(project).then(function (items) {
			$scope.todoist = items;
		}, function (error) {
	...
	var addTodoist = function(element, project){
		// TodoistService.addTask(element, project);
		// getTodoist(project);
